...
if (isset($_POST['action']) && $_POST['action'] == 'generate_token' && isset($_POST['nid'])) {
    ...
    $update_query = "UPDATE doctor SET login_token='$token' WHERE NID='$nid'";
...
if (isset($_GET['action']) && $_GET['action'] == 'reject' && isset($_GET['NID'])) {
    ...
    $update_query = "UPDATE doctor SET Status='Rejected' WHERE NID='$nid'";
    if (mysqli_query($conn, $update_query)) {
        header("Location: managed_doctors.php?status=rejected");
        exit();
    }
}

// শুধুমাত্র 'Approved' ডাক্তারদের ডাটা তুলে আনা
$query = "SELECT * FROM doctor WHERE Status='Approved' ORDER BY Reg_Date DESC";
...
<head>
    ...
    <title>Admin - Managed Approved Doctors</title>
...
<main class="container my-4">
    ...
    <?php if (isset($_GET['status']) && $_GET['status'] == 'rejected'): ?>
        <div class="alert alert-warning alert-dismissible fade show">
            <i class="bi bi-exclamation-triangle-fill"></i> Doctor Status Updated to Rejected & Removed from Approved List!
            <button class="btn-close" data-bs-dismiss="alert"></button>
        </div>
    <?php endif; ?>

    <div class="d-flex justify-content-between align-items-center mb-4">
        <h2 class="text-secondary fw-bold mb-0"><i class="bi bi-person-badge-fill text-success"></i> Approved Doctors Management</h2>
    ...
    <div class="card shadow-sm">
        <div class="card-header bg-success text-white py-3">
            <h4 class="mb-0 fw-bold"><i class="bi bi-check-circle-fill"></i> Active & Verified Doctors</h4>
        </div>
        <div class="card-body">
            <div class="table-responsive">
                <table class="table table-striped table-hover align-middle dynamicTable w-100">
                    ...
                    <tbody>
                        <?php while ($row = mysqli_fetch_assoc($result)) { $row['Role'] = 'Doctor'; ?>
                        <tr>
                            <td><strong><?= htmlspecialchars($row['Full_Name']) ?></strong></td>
                            <td><?= htmlspecialchars($row['NID']) ?></td>
                            <td><span class="badge bg-secondary"><?= htmlspecialchars($row['Specialization']) ?></span></td>
                            ...
                            <td class="text-center">
                                ...
                                <a href="managed_doctors.php?action=reject&NID=<?= urlencode($row['NID']) ?>" class="btn btn-sm btn-danger" onclick="return confirm('Are you sure?')">
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            <div class="modal-header bg-info text-white">
                <h5 class="modal-title fw-bold" id="modalRole"><i class="bi bi-person-vcard"></i> Doctor Profile Details</h5>
...
$(document).ready(function() {
    $('.dynamicTable').DataTable({
        "pageLength": 10,
        "language": { "emptyTable": "No approved doctors found." }
    });

    // ওয়ান-টাইম সিকিউর ইনস্ট্যান্ট অটো লগইন লজিক
    $(document).on('click', '.autologin-btn', function() {
        var doctorNid = $(this).data('nid');
        var clickedBtn = $(this);
        
        clickedBtn.prop('disabled', true).html('<span class="spinner-border spinner-border-sm"></span> Connecting...');

        $.ajax({
            url: 'managed_doctors.php',
            type: 'POST',
            data: { action: 'generate_token', nid: doctorNid },
            dataType: 'json',
            success: function(res) {
                if(res.status === 'success') {
                    var loginUrl = "admin_autologin.php?role=doctor&NID=" + encodeURIComponent(doctorNid) + "&token=" + res.token;
    ...
    $(document).on('click', '.view-user', function() {
        ...
        var htmlContent = `
            <table class="table table-bordered table-striped">
                ...
                <tr><th>Specialization</th><td><span class="badge bg-secondary">${data.Specialization || 'N/A'}</span></td></tr>
